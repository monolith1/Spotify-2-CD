...
BASE_CD_DIR = r"C:\Users\monol\wkdir\monosound\cd"
...
def get_orders():
    ...
    while True:
        ...
        folder_name = f"cd_{date_str}_{customer_name}"
        full_path = os.path.join(BASE_CD_DIR, folder_name)
        orders.append((link, full_path, customer_name))
    return orders
...
def download_playlist(link, output_folder):
    ...
    cmd = [
        "spotdl", link, 
        "--output", f"{output_folder}/{{track-number}} - {{artist}} - {{title}}.{{output-ext}}"
    ]
    try:
...
def generate_cue_sheet(folder_path, customer_name):
    """
    ...
    with open(cue_path, "w", encoding="utf-8") as cue:
        # Album Metadata
        cue.write(f'TITLE "Mix for {customer_name}"\n')
        cue.write(f'PERFORMER "Compilation"\n')
        cue.write(f'FILE "" WAVE\n\n') # Header
...
def generate_tracklist(folder_path, customer_name):
    """
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write(f"Mix for {customer_name}\n")
        f.write("-" * 40 + "\n")
...
def main():
    ...
    for link, folder, customer_name in orders:
        print(f"\n=== Processing: {folder} ===")
        # 1. Download
        ...
        # 3. Create CUE Sheet
        cue_file = generate_cue_sheet(folder, customer_name)
        generate_tracklist(folder, customer_name)
        ready_to_burn.append((folder, cue_file))
    
    # Phase 2: Interactive Burn
    print(f"\n=== Batch Processing Complete. {len(ready_to_burn)} orders ready. ===")
    
    for folder, cue_file in ready_to_burn:
        print(f"\n>>> Burning Order: {folder}")
